"""Postgres-backed warehouse tools for the ADK agent (check availability, reserve).

Used by ``warehouse_manager_agent.agent`` whether the agent runs in a notebook,
``adk web``, or this **A2A** server—the tools are plain Python functions the LLM
invokes through the ADK tool-calling layer.

Connects to ``tools_database`` on ``localhost:5433`` — same stack as Week 5/6
notebooks when Docker Compose Postgres is up (``langgraph_user`` /
``langgraph_password``). **Learning / dev credentials** only; production would
use env-driven DSNs and least-privilege DB users.
"""
import psycopg2
