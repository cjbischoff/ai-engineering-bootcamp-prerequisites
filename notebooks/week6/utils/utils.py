"""
Utilities for LangGraph multi-turn agents (Week 4–6).

Used by Week 4–5 notebooks and Week 6 (`notebooks/week6/01-litellm-router.ipynb`)
via the local `utils` package. Serializes agent tool calls into `AIMessage` and
builds tool metadata from function source for coordinator prompts.

`format_ai_message`: each turn needs fresh `tool_call` IDs for OpenAI-compatible APIs.
`get_tool_descriptions` / `parse_function_definition`: AST + docstring parsing to avoid
duplicating tool schemas in prose.
"""
